package com.amitesh.portfolio.newsroomelements.service;

import com.amitesh.portfolio.newsroomelements.api.CreateRecordRequest;
import com.amitesh.portfolio.newsroomelements.domain.PortfolioRecord;
...
public class PortfolioService {
    private final CopyOnWriteArrayList<PortfolioRecord> records = new CopyOnWriteArrayList<>(List.of(
            new PortfolioRecord(UUID.randomUUID(), "Rights review", "IN_REVIEW", "Publishing task", Instant.now()),
            new PortfolioRecord(UUID.randomUUID(), "French caption check", "READY", "Publishing task", Instant.now()),
            new PortfolioRecord(UUID.randomUUID(), "Homepage release approval", "OPEN", "Publishing task", Instant.now())
    ));
